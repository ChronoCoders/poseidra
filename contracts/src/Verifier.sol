...
/// @notice Common interface for PLONK verifiers.
///         proof is a 24-element uint256 array matching snarkjs PLONK proof encoding.
///         pubSignals order: [root, nullifier_hash, recipient, chain_id, contract_address]
interface IVerifier {
    function verifyProof(uint256[24] calldata proof, uint256[5] calldata pubSignals)
        external view returns (bool);
}

/// @notice Stub verifier — always accepts. Used in tests and deployment before real verifier.
contract VerifierStub is IVerifier {
    function verifyProof(uint256[24] calldata, uint256[5] calldata)
        external pure returns (bool) { return true; }
}

// ─── Generated PlonkVerifier ──────────────────────────────────────────────────
...
contract PlonkVerifier is IVerifier {
    ...
    uint256 constant Qmx  = 8553711069428273830576252956834510860019693672612633688733363966669694837988;
    uint256 constant Qmy  = 9711752221909437864384152718980601592557406758666663303554652285965296475835;
    uint256 constant Qlx  = 13539211466694083262168553995401718883681135275436706332607266981881941462020;
    uint256 constant Qly  = 15774960986360204382927684130943689865038897413246436185168562434054586067317;
    uint256 constant Qrx  = 2630014660531688498548655490525652321382871217229130293028485530358366970419;
    uint256 constant Qry  = 15498233788274098115879884001718227329392151430607727008822086063862467779674;
    uint256 constant Qox  = 12102225460303552950542858491564103306608615729156275486681632192870994749976;
    uint256 constant Qoy  = 10609913031674010280326225580743192814792228508602550647629474288273636851716;
    uint256 constant Qcx  = 2258956683081365378193821055594879866707696777934795069578770596079052991517;
    uint256 constant Qcy  = 11725793730266759595704221591780435940638113637171537425729948524715601190322;
    uint256 constant S1x  = 576897524886874605507050901521257042283087386041140508099071914220576810590;
    uint256 constant S1y  = 20154054906213782997340456295409787894901628850257513826473489886740568021409;
    uint256 constant S2x  = 4351708301750270504926742154640565673146323958889550501261382505335159640928;
    uint256 constant S2y  = 20215669528089660829722293307687241036634763620978475531320538010385411186958;
    uint256 constant S3x  = 2584677652783455626761348757325787147295533481643668434264618208137786419469;
    uint256 constant S3y  = 8719838541288115536437527018605714874824843483655854837958466019290818278173;
    uint256 constant k1   = 2;
    uint256 constant k2   = 3;
    uint256 constant X2x1 = 17644969763784577319871161861128937703249540589619487993916387281891824818326;
    uint256 constant X2x2 = 10098197737731323103829925427551042944657354283878778429126401998505089310288;
    uint256 constant X2y1 = 1028245025968242606791338297484769089558406274103468408926663877128746263746;
    uint256 constant X2y2 = 13139361876026080358876511140390101221496608963166104728021242535685596611806;
    ...
    function verifyProof(uint256[24] calldata, uint256[5] calldata _pubSignals) public view override returns (bool) {
        assembly {
            /////////
